...
across 318 designs; three designs additionally have BLI replicates).
Per-replicate KD fits were averaged after curated-replicate filtering
at ProteinBase.
...
\texttt{data/designs.csv}). The \emph{Strong} bin maps to KD
\textless{} 100 nM.
...
Headline counts and rates are reported with raw n/N. Binary comparisons
(e.g.~method-family hit rates) used Fisher's exact (two-sided) with
p-value and odds ratio reported together. With 9 binders across 322
designs, almost every per-method or per-modality split is underpowered.
